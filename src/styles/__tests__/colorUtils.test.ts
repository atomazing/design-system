import { getFontColor, isFontLight, isHexColor } from "../colorUtils";
...
describe("isDark", () => {
  it("returns true for dark colors", () => {
    expect(isFontLight("#202020")).toBe(true);
    expect(isFontLight("#7F7F7F")).toBe(true); // Near-threshold gray
  });

  it("returns false for light colors", () => {
    expect(isFontLight("#F0F0F0")).toBe(false);
    expect(isFontLight("#abcdef")).toBe(false);
  });
});
